...
async function testSearchIndex() {
  ...
  if (stats.totalDocuments < 10) {
    throw new Error(
      `Only ${stats.totalDocuments} documents indexed, expected more`
    );
  }
...
async function testTonApiService() {
  ...
  const valid = tonApiService.validateAddress(
...
async function testDocumentationAccess() {
  const { readMarkdownFromDirectory, getAvailableHowToResources } =
